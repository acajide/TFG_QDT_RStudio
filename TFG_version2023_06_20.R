...
#nrow(grupo1_cluster1)
#nrow(grupo2_cluster1)
#nrow(grupo3_cluster1)
...
print(model_cluster1)

#Imprimir los valores reales y la media
#model_cluster1$data$Portugues_distancia
...
#model_cluster1$data$edad_Cat

#imprimir los valores del fit de la función y la media
#model_cluster1$fitted.values
...
mean(model_01_cluster1$data$Portugues_distancia) # = 0.3860759
...
#Tranajar con el 10% de cada cluster que nos queda. 1º vamos a hallar la probabilidad 
# empírica de cada uno de ellos. 
...
grupo1 <- cluster1 %>% sample_frac(0.7)  # 70% de los datos
grupo2 <- cluster1 %>% anti_join(grupo1) %>% sample_frac(2/3)  # 20% de los datos
grupo3 <- cluster1 %>% anti_join(grupo1) %>% anti_join(grupo2)  # 10% de los datos
...
#Modelo
model_cluster1<-glm(Portugues_distancia~Sexo+edad_Cat+Agrupaciones+Medio_binaria,data=grupo1,family = binomial(link = "logit"))
print(model_cluster1)

#Imprimir los valores empíricos y la media
model_cluster1$data$Portugues_distancia
mean(model_cluster1$data$Portugues_distancia) #p(x) = 0,5804702
...
# Aplicar el modelo a grupo2
grupo1_pred <- predict(model_cluster1, newdata = grupo2, type = "response")

# Imprimir los resultados
print(grupo1_pred)
mean(grupo1_pred) # = 0.5760655 <- f(Lj)

#vamos a mirar cuales son los datos del grupo dos para comparar con nuestra prediccion
model_01_cluster1<-glm(Portugues_distancia~Sexo+edad_Cat+Agrupaciones+Medio_binaria,data=grupo2,family = binomial(link = "logit"))
print(model_01_cluster1)

#imprimos los valores de 0 o 1 que nos dan en el 20%
model_01_cluster1$data$Portugues_distancia
mean(model_01_cluster1$data$Portugues_distancia) # = 0.61392441 p(x)

#hallar la diferencia entre lo empirico y lo predicho
q <- mean(model_01_cluster1$data$Portugues_distancia) - mean(grupo1_pred)
print(q) # q = 0.03785875
